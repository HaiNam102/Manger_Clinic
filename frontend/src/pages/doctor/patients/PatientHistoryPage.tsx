...
const PatientHistoryPage = () => {
    ...
    if (!patient) return <div className="text-center py-24 text-dark-400 bg-dark-900/20 rounded-3xl border border-dashed border-dark-700 m-6">Không tìm thấy thông tin bệnh nhân</div>;
        ...
        <div className="space-y-6 animate-fade-in pb-10">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-4">
                    <Link
                        to="/doctor/patients"
                        className="h-10 w-10 flex items-center justify-center rounded-xl bg-dark-800 border border-dark-700 text-dark-300 hover:text-primary-400 hover:border-primary-700/50 transition-all shadow-lg active:scale-95"
                    >
                        <ArrowLeft size={20} />
                    </Link>
                    <div>
                        <div className="flex items-center gap-2">
                            <h1 className="text-2xl font-bold text-dark-50">Chi tiết bệnh nhân</h1>
                        ...
                        <p className="text-dark-400 text-sm mt-0.5">Mã BN: <span className="font-mono text-dark-200 uppercase">{patient.id.slice(-8)}</span></p>
                    </div>
                </div>
                <Button
                    variant="primary"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-4">
                    <div className="flex items-center justify-between px-2">
                        ...
                        <span className="text-xs font-medium text-dark-400 bg-dark-800 px-3 py-1 rounded-full border border-dark-700/50">
                            {records.length} phiên khám
                        </span>
                    </div>

                    {records.length === 0 ? (
                        <Card className="bg-dark-900/30 border-dashed">
                            <CardContent className="py-20 text-center">
                                <FileText size={48} className="mx-auto text-dark-700 mb-4 opacity-50" />
                                <p className="text-dark-400 font-medium italic">Bệnh nhân chưa có lịch sử khám bệnh tại phòng khám.</p>
                            </CardContent>
                        </Card>
                    ) : (
                        ...
                        <CardContent className="space-y-4">
                            <div>
                                ...
                                {patient.allergies && patient.allergies.length > 0 ? (
                                    ...
                                    <p className="text-xs text-dark-400 italic">Không có thông tin dị ứng</p>
                                )}
                            </div>
                        </CardContent>
                    </Card>
                            ...
                            <CardContent>
                                <div className="grid grid-cols-2 gap-3">
                                    {Object.entries(records[0].vitalSigns).map(([label, value]) => (
                                        <div key={label} className="bg-dark-900/50 p-2.5 rounded-xl border border-sky-800/10 hover:border-sky-500/20 transition-colors">
                                            <p className="text-[9px] text-dark-500 uppercase font-black tracking-widest mb-1">
...
const PatientHeroCard = ({ patient, getAge }: { patient: PatientResponse; getAge: (d: string) => number }) => (
    <Card className="border-dark-700/50 bg-dark-950/80 shadow-2xl overflow-hidden relative">
        ...
        <CardContent className="p-0">
            <div className="flex flex-col lg:flex-row border-l-4 border-l-primary-600">
                {/* Profile Section */}
                <div className="p-6 lg:p-8 flex flex-col md:flex-row items-center gap-6 lg:border-r lg:border-r-dark-800 flex-1">
                    <div className="h-24 w-24 rounded-3xl bg-gradient-to-br from-primary-600 to-indigo-700 flex items-center justify-center shadow-2xl relative">
                        ...
                        <div className="absolute -bottom-2 -right-2 h-8 w-8 rounded-xl bg-dark-900 border-2 border-primary-600 flex items-center justify-center">
                            <UserCheck size={16} className="text-primary-500" />
                        </div>
                    </div>

                    <div className="flex-1 text-center md:text-left">
                        <h2 className="text-3xl font-black text-dark-50 tracking-tight">{patient.fullName}</h2>
                        <div className="flex flex-wrap justify-center md:justify-start items-center gap-3 mt-2">
                            <Badge variant="default" size="sm" className="bg-dark-800 text-dark-300 border-dark-700 font-bold px-3">
                ...
                {/* Contact Section */}
                <div className="p-6 lg:p-8 grid grid-cols-1 md:grid-cols-2 gap-4 bg-dark-900/20 lg:w-1/3">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3 text-dark-400">
                            <div className="h-8 w-8 rounded-lg bg-dark-800 flex items-center justify-center text-dark-500 border border-dark-700"><Phone size={14} /></div>
                            <div>
                                <p className="text-[10px] uppercase font-bold text-dark-500">Số điện thoại</p>
                                <p className="text-sm font-semibold text-dark-200">{patient.phoneNumber || 'N/A'}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 text-dark-400">
                            <div className="h-8 w-8 rounded-lg bg-dark-800 flex items-center justify-center text-dark-500 border border-dark-700"><Mail size={14} /></div>
                            <div>
                                <p className="text-[10px] uppercase font-bold text-dark-500">Email</p>
                                <p className="text-xs font-medium text-dark-300 truncate max-w-[150px]">{patient.email}</p>
                            </div>
                        </div>
                    </div>
                    <div className="space-y-4">
                        <div className="flex items-center gap-3 text-dark-400">
                            <div className="h-8 w-8 rounded-lg bg-dark-800 flex items-center justify-center text-dark-500 border border-dark-700"><Calendar size={14} /></div>
                            <div>
                                <p className="text-[10px] uppercase font-bold text-dark-500">Ngày sinh</p>
                                <p className="text-sm font-semibold text-dark-200">{patient.dateOfBirth ? new Date(patient.dateOfBirth).toLocaleDateString('vi-VN') : 'N/A'}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 text-dark-400">
                            <div className="h-8 w-8 rounded-lg bg-dark-800 flex items-center justify-center text-dark-500 border border-dark-700"><MapPin size={14} /></div>
                            <div>
                                <p className="text-[10px] uppercase font-bold text-dark-500">Địa chỉ</p>
                                <p className="text-sm font-semibold text-dark-200 truncate max-w-[150px]">{patient.address || 'Hà Nội, Việt Nam'}</p>
                            </div>
                        </div>
                    </div>
                </div>
...
const TimelineItem = ({ record, isExpanded, onToggle, onEdit }: { record: MedicalRecordResponse; isExpanded: boolean; onToggle: () => void; onEdit: () => void }) => (
    <div className={`border rounded-2xl overflow-hidden transition-all duration-300 ${isExpanded ? 'border-primary-700/50 bg-dark-900/40 shadow-2xl' : 'border-dark-700/50 hover:bg-dark-800/30'}`}>
        <button onClick={onToggle} className="w-full flex items-center gap-4 p-4 text-left group">
            <div className={`h-12 w-12 rounded-2xl flex items-center justify-center flex-shrink-0 transition-all ${isExpanded ? 'bg-primary-600 shadow-lg shadow-primary-900/20 text-white' : 'bg-dark-800 text-primary-400 border border-dark-700 group-hover:border-primary-600/50'}`}>
                <Stethoscope size={20} />
            </div>

            <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                    <p className="text-base font-bold text-dark-50 tracking-tight">{record.diagnosis}</p>
                ...
                <div className="flex items-center gap-3 mt-1">
                    <p className="text-xs font-bold text-dark-500 flex items-center gap-1.5">
                        <Calendar size={12} />
                        {new Date(record.createdAt).toLocaleDateString('vi-VN', { weekday: 'long', day: '2-digit', month: '2-digit', year: 'numeric' })}
                    </p>
                    <span className="text-dark-700">•</span>
                    <p className="text-xs font-medium text-dark-400">Bác sĩ: <span className="text-dark-200">{record.doctorName || 'Bs. Đang cập nhật'}</span></p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3 pr-2">
                <button
                    ...
                    className="h-9 w-9 flex items-center justify-center rounded-xl text-dark-500 hover:text-primary-400 hover:bg-primary-900/20 transition-all border border-transparent hover:border-primary-800/50"
                    title="Chỉnh sửa ghi chú"
                >
                    <Edit3 size={15} />
                </button>
                <div className={`h-8 w-8 flex items-center justify-center rounded-full transition-all ${isExpanded ? 'bg-primary-900/20 text-primary-400 rotate-180' : 'text-dark-600'}`}>
                    <ChevronDown size={18} />
                </div>
            </div>
        </button>
...
const DetailedRecordView = ({ record }: { record: MedicalRecordResponse }) => (
    <div className="border-t border-dark-700/30 p-6 bg-dark-950/20 animate-fade-in space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <section>
                <h4 className="text-[10px] font-black text-dark-500 uppercase tracking-[2px] mb-3 flex items-center gap-2">
                    <div className="h-1.5 w-1.5 rounded-full bg-primary-500" /> Triệu chứng & Khám lâm sàng
                </h4>
                <div className="bg-dark-900/40 rounded-2xl p-4 border border-dark-800/60 leading-relaxed text-dark-200 text-sm">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {record.vitalSigns && typeof record.vitalSigns === 'object' ? (
                        Object.entries(record.vitalSigns).map(([label, value]) => (
                            <div key={label} className="bg-dark-900/60 p-3 rounded-2xl border border-dark-800 shadow-inner flex flex-col items-center text-center">
                                <p className="text-[9px] text-dark-500 uppercase font-black leading-tight mb-1">
                                    {label === 'bloodPressure' ? 'Huyết áp' : label === 'heartRate' ? 'Nhịp tim' : label === 'temperature' ? 'Nhiệt độ' : label === 'weight' ? 'Cân nặng' : label.replace(/([A-Z])/g, ' $1').trim()}
                                </p>
                        ...
                        <div className="col-span-full py-4 text-center text-xs text-dark-500 italic">Không có dữ liệu sinh hiệu.</div>
                    )}
                </div>
            </section>
        </div>
        ...
        <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-indigo-900/10 border border-indigo-900/20 rounded-2xl p-4">
                    ...
                    <p className="text-base font-bold text-dark-50">{record.diagnosis}</p>
                </div>
                <div className="bg-emerald-900/5 border border-emerald-900/10 rounded-2xl p-4">
                    ...
                    <p className="text-sm text-dark-300">{record.treatment || "Theo dõi và điều trị triệu chứng."}</p>
                </div>
            </div>
        </section>
        ...
        {record.notes && (
            <section>
                <h4 className="text-[10px] font-black text-dark-500 uppercase tracking-[2px] mb-3 flex items-center gap-2">
                    <div className="h-1.5 w-1.5 rounded-full bg-dark-500" /> Ghi chú bác sĩ
                </h4>
                <p className="text-xs text-dark-400 italic bg-dark-900/30 p-3 rounded-xl border border-dark-800 border-l-4 border-l-dark-700">
                    "{record.notes}"
                </p>
            </section>
        )}
                ...
                <div className="border border-emerald-900/20 rounded-3xl p-6 bg-emerald-950/5 relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between mb-6 relative">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h5 className="text-lg font-black text-dark-50">Toa thuốc điện tử</h5>
                        ...
                        {record.prescription.validUntil && (
                            <div className="text-right">
                                <p className="text-[10px] font-bold text-dark-500 uppercase tracking-widest">Có giá trị đến</p>
                                <p className="text-xs font-black text-dark-200">{new Date(record.prescription.validUntil).toLocaleDateString('vi-VN')}</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="overflow-x-auto relative">
                        <table className="w-full">
                            <thead>
                                <tr className="text-[10px] text-dark-500 uppercase font-black tracking-widest border-b border-dark-800">
                            ...
                            <tbody className="divide-y divide-dark-800/50">
                                {record.prescription.details?.map((item, i) => (
                                    <tr key={item.id || i} className="group hover:bg-dark-800/20 transition-colors">
                                        <td className="py-4 px-2 text-xs font-mono text-dark-600">{String(i + 1).padStart(2, '0')}</td>
                                        <td className="py-4 px-2">
                                            <p className="text-sm font-black text-dark-100 group-hover:text-emerald-400 transition-colors">{item.medicineName}</p>
                                            <p className="text-[10px] text-dark-500 italic mt-0.5">{item.instructions || "Dùng theo chỉ định."}</p>
                                        </td>
                                        <td className="py-4 px-2 text-xs text-dark-300 font-bold">{item.dosage}</td>
                                        <td className="py-4 px-2 text-xs text-dark-400">{item.frequency} <span className="text-dark-600">trong</span> {item.duration}</td>
                    ...
                    {record.prescription.notes && (
                        <div className="mt-6 pt-4 border-t border-dark-800 flex items-start gap-2">
                            <span className="text-emerald-500 text-lg leading-none mt-0.5">“</span>
                            <p className="text-xs text-dark-400 italic">{record.prescription.notes}</p>
                        </div>
                    )}
                </div>
            </section>
